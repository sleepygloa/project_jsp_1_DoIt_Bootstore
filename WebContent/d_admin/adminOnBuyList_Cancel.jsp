		...
		<p class="num_tag">
			<c:if test="${count > 0}">
				...
				<c:if test="${startPage > 10}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${startPage - 10 }&cenList=cell">[ 이전 ]</a>
				</c:if>
			
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${i}&cenList=cell">[ ${i} ]</a>
				</c:forEach>
			
				<c:if test="${endPage < pageCount}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${startPage + 10}&cenList=cell">[ 다음 ]</a>
				</c:if>
			</c:if>
