...
<jsp:include page="/d_admin/adminSide.jsp" />
	...
	<p id="AdminList_title"> 책 등록 리스트</p>
	...
	<table id="AdminList_Wrap">
		<tr class="AdminList_tr">
			...
			<td class="AdminList_Ins Admin_txtCen">상세보기</td>	
		</tr>	
			
		<c:forEach var="article"  items="${articleList}">	
			<form action="/DoIt/d_admin/adminOnBookList_detail.do" method="post">
				<tr class="AdminList_tr">
					<td class="AdminList_num Admin_txtCen"> <!--  책이미지와 셀의 높이가 10px 커야함. thumbnail 적용 여백 -->
					...
					<td  class="AdminList_pic Admin_txtCen">
						...
						<c:if test="${article.d_bpic != null}">
							<a href="/DoIt/d_admin/admin.do"><img src="/DoIt/d_bpic/${article.d_bpic}" height="190px;" class="thumbnail" /></a>
						</c:if>
					</td>
					<td class="AdminList_con">
						<div class="AdminList_con_list">
				 				책 코드 : ${article.d_bcode} <br/>
				 				책 이름 : ${article.d_bname} <br/>
				 				책등급 : ${article.d_bgrade}
				 				저자 : ${article.d_bauthor} <br/>
				 				출판사 : ${article.d_bpublisher} <br/>
				 				책 등록 날짜 : ${article.d_bdate} <br /><br />
				 				<input type="hidden" name="d_bcode" value="${article.d_bcode}">
						</div>
					</td>	
					<td class="AdminList_Ins Admin_txtCen"> 
		 				<input type="submit" value="상세보기" class="AdminList_btn"/>
		 			</td>
	...
	<div id="AdminList_PageNum">
		<c:if test="${count > 0}">
			...
			<c:if test="${startPage > 10}">
				<a href="/DoIt/d_admin/adminOnBookList.do?pageNum=${startPage - 10 }">[이전]</a>
			</c:if>
		
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="/DoIt/d_admin/adminOnBookList.do?pageNum=${i}">[${i}]</a>
			</c:forEach>
		
			<c:if test="${endPage < pageCount}">
				<a href="/DoIt/d_admin/adminOnBookList.do?pageNum=${startPage + 10}">[다음]</a>
			</c:if>
		</c:if>
	</div>
