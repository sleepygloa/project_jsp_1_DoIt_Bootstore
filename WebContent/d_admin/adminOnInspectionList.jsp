...
<article class="my_cont_wrap">
	<p id="AdminList_title"> 회원 판매 신청완료 리스트</p>
	...
	<table id="AdminList_Wrap">	
		<tr class="AdminList_tr">
			...
			<td class="AdminList_Ins Admin_txtCen">책 등록</td>	
		</tr>
	...
	<div id="AdminList_PageNum">
		<c:if test="${count > 0}">
				...
				<c:if test="${startPage > 10}">
					<a href="/DoIt/d_admin/adminOnInspectionList.do?pageNum=${startPage - 10 }">[이전]</a>
				</c:if>
			
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="/DoIt/d_admin/adminOnInspectionList.do?pageNum=${i}">[${i}]</a>
				</c:forEach>
			
				<c:if test="${endPage < pageCount}">
					<a href="/DoIt/d_admin/adminOnInspectionList.do?pageNum=${startPage + 10}">[다음]</a>
				</c:if>
			</c:if>
		</div>
