		...
		<p class="num_tag">		
			<c:if test="${count > 0}">
				...
				<c:if test="${startPage > 10}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${startPage - 10 }&ynoo=nooo">[ 이전 ]</a>
				</c:if>
			
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${i}&ynoo=nooo">[ ${i} ]</a>
				</c:forEach>
			
				<c:if test="${endPage < pageCount}">
					<a href="/DoIt/d_manage/manPart1.do?pageNum=${startPage + 10}&ynoo=nooo">[ 다음 ]</a>
				</c:if>
			</c:if>
		</p>
