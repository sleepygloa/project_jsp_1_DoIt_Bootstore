...
<%-- 연체자 리스트 및 연체 처리 --%>
	...
	<c:if test="${count > 0 }">
		...
		<%-- 이동 버튼 --%>
		<p class="write_bu">
			<button onclick="window.location='/DoIt/d_manage/manPart2.do'" >전체보기</button>
			<button onclick="window.location='/DoIt/d_manage/manPart2.do?waitB=no'" >승인 대기중 상태</button>
		</p>
		...
		<p class="num_tag">
		   ...
		   <c:set var="endPage" value="${startPage + pageBlock-1}"/>
		        ...
		        <c:if test="${ startPage>10 }">
		        	<a href="/DoIt/d_manage/manPart2.do?pageNum=${startPage - 10 }">[ 이전 ]</a>
				</c:if>
				<c:forEach var="i" begin="${ startPage}" end="${endPage}">
		        	<a href="/DoIt/d_manage/manPart2.do?pageNum=${ i }">[ ${ i } ]</a>
				</c:forEach>
				<c:if test="${endPage < pageCount }">
		        	<a href="/DoIt/d_manage/manPart2.do?pageNum=${ startPage + 10 }">[ 다음 ]</a>
				</c:if>
